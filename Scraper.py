...
def extract_episode_urls(page, target_url, config, selector_key='anime_main'):
    bypass_js = config.get("bypass_javascript", False)
    selectors = config.get("selectors", {})
    target_selectors = selectors.get(selector_key, {})
    episodes_section_selector = target_selectors.get('episodes_section')

    if not episodes_section_selector:
        logging.error(f"Configuração '{selector_key}.episodes_section' ausente ou inválida.")
        return {'url': target_url, 'error': f"Configuração '{selector_key}.episodes_section' não encontrada."}

    try:
        if bypass_js:
            logging.info(f"bypass_javascript = true => Injetando iframe para {target_url}")
            episode_urls = bypass_inject_iframe_and_get_episode_links(page, target_url, episodes_section_selector)
            logging.info(f"Encontradas {len(episode_urls)} URLs de episódios (via bypass).")
            return {'url': target_url, 'episode_urls': episode_urls}
        else:
            page.goto(target_url, timeout=60000)
            page_title = page.title()
            logging.info(f"Título da Página: {page_title}")
        ...
        logging.error(f"Timeout ao acessar {target_url}: {str(e)}")
        capture_screenshot(page, target_url, prefix="url_timeout")
        return {'url': target_url, 'error': f'Timeout: {str(e)}'}
    except Exception as e:
        logging.error(f"Erro ao extrair episodios de {target_url}: {str(e)}")
        capture_screenshot(page, target_url, prefix="url_exception")
        return {'url': target_url, 'error': f'Erro geral: {str(e)}'}
...
def get_embed():
    ...
    try:
        if match_url_pattern(input_url, url_patterns.get('home', '')):
            logging.info(f'Processando página inicial: {input_url}')
            episodes_info = extract_episode_urls(page, input_url, config, selector_key='home')
        ...
        elif match_url_pattern(input_url, url_patterns.get('anime_main', '')):
